...
class NorenApi:
    ...
    def __ws_run_forever(self):
        # Note: 
        # Why the changes:
        #   Whenever run_forever throws exception, it tears down websocket connection. Some time even before the 
        #   socket is closed, exception can occur. At that time, socket is open and this code 
        #   trys to open another socket which fails as it is already opened. For this added, 
        #
        #         except websocket.WebSocketException:
        #           self.__websocket.close()
        #   
        #   Actually, this while loop it self is not required.  Just to keep the code as close 
        #   to finvasia, keeping it.
        #   skip_utf8_validation=True gives some performance enhancement. 
        #
        #   Details:  https://github.com/websocket-client/websocket-client?tab=readme-ov-file#performance
        #    
        while self.__stop_event.is_set() == False:
            try:
                ...
                if not errored:
                    ...
                    logger.warning (f'run_forever: Did not Exit Cleanly' )
    ...
    def __on_data_callback(self, ws=None, message=None, data_type=None, continue_flag=None):
        ...
        # Note: 
        # Why the changes:
        # Minor performance improvement. If there are high ticks per sec coming through the websocket
        # This will help. Sequence of the conditions are ticks, orderupdate, error and then open
        # which is based on high probability of occcurence.

        try:
            res = json.loads(message)
        except Exception as e:
            logger.info(f'json.loads(message) Exception {str(e)}')
        else:
            if res['t'] in {'tk', 'tf', 'dk', 'df'} and self.__subscribe_callback:
                self.__subscribe_callback(res)
                return

            if res['t'] == 'om' and self.__order_update_callback:
                self.__order_update_callback(res)
                return

            if res['t'] == 'ck':
                if res['s'] != 'OK' and self.__on_error:
                    self.__on_error(res)
                    return
                elif res['s'] == 'OK' and self.__on_open:
                    self.__on_open()
                    return
    ...
    def start_websocket(self, subscribe_callback = None, 
        ...
        websocket.setReconnect(2)
        
        # Following will be useful for getting the line data
